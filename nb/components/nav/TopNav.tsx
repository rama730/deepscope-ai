...
import { Menu, Compass, LayoutGrid, Users, Settings, MessageSquare } from "lucide-react";
...
import { useWorkspace } from "@/components/workspace-v2";
...
export default function TopNav() {
  ...
  const prefetchHub = useCallback(() => {
    // Default filters matching the initial state of Hub page
    ...
    queryClient.prefetchInfiniteQuery({
      queryKey: hubKeys.list('all', defaultFilters),
          ...
          {navItems.map((item) => {
            ...
            return (
              <NavLink
                ...
                isActive={!!isActive}
                badge={badge}
                onMouseEnter={item.href === ROUTES.HUB ? prefetchHub : undefined}
              />
            );
          })}
        </nav>
