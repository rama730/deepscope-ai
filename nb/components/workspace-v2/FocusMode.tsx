...
function ProjectTaskRow({
  ...
  return (
    <motion.div 
      ...
      className="group relative flex items-start gap-3 p-3 rounded-xl border border-zinc-200 dark:border-zinc-800 bg-white dark:bg-zinc-900 hover:shadow-md transition-all"
      role="button"
      tabIndex={0}
        ...
        <div className="flex items-center justify-between mb-1">
          ...
          {task.due && (
            <span className={cn(
              ...
              task.due === "Today" || task.due.includes(new Date().getFullYear().toString()) // Simple check
...
function FocusMode() {
  ...
  return (
    <div className="flex flex-col gap-8 h-full">
      ...
      {/* --- Section 1: Daily Routine (Notion Style) --- */}
        ...
        <div className="bg-white/50 dark:bg-zinc-900/30 rounded-xl p-2 border border-dashed border-zinc-200 dark:border-zinc-800">
          ...
          {isAddingHabit ? (
            ...
            <button 
              ...
              className="mt-2 w-full flex items-center gap-2 p-2 text-zinc-400 hover:text-zinc-600 dark:text-zinc-400 dark:hover:text-zinc-300 text-sm rounded-lg hover:bg-zinc-50 dark:bg-zinc-900 dark:hover:bg-zinc-800/50 transition-colors"
            >
              <Plus size={16} />
              <span>Add habit</span>
            </button>
            ...
            <AnimatePresence mode="popLayout">
              {tasks.map(task => (
                <SwipeableRow
                  key={task.id}
                  canSwipe
                  onSwipeRight={task.status === "todo" ? () => startTask(task.id) : undefined}
                  onSwipeLeft={() => completeTask(task.id)}
                  rightLabel={task.status === "todo" ? "Start" : undefined}
                  leftLabel="Done"
