...
function InboxCard({ item, onSwipe }: { item: InboxItem; onSwipe: (id: string, action: 'complete' | 'snooze') => void }) {
  ...
  return (
    ...
    >
      {/* Background Actions */}
      <div className="absolute inset-0 rounded-xl overflow-hidden flex items-center justify-between px-6 pointer-events-none">
        <div className="flex items-center gap-2 text-emerald-600 font-bold opacity-0 group-hover:opacity-100 transition-opacity" style={{ opacity: x.get() > 50 ? 1 : 0 }}>
          <Check size={20} />
          Mark Read
        </div>
        <div className="flex items-center gap-2 text-amber-600 font-bold opacity-0 group-hover:opacity-100 transition-opacity" style={{ opacity: x.get() < -50 ? 1 : 0 }}>
          Snooze
          <Clock size={20} />
        </div>
      </div>
...
function TriageMode() {
  ...
  const handleSwipe = (id: string, action: 'complete' | 'snooze') => {
    if (action === 'complete') {
      ...
      // For now, snooze just acts like dismiss in this demo, 
      // but ideally it would re-surface later.
