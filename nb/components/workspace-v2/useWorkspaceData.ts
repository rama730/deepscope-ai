...
export function useWorkspaceData() {
  ...
  useEffect(() => {
    ...
    if (savedHabits) {
      try {
        ...
        // Reset completed status if it's a new day (simple check)
        // For a real app, we'd store 'lastCompletedDate' per habit
        setHabits(parsed);
      } catch (e) {
    ...
    try {
      ...
      if (!tasksError && tasksData) {
        const mappedTasks: WorkspaceTask[] = tasksData.map((t: any) => ({
          ...
          projectColor: "bg-indigo-500", // You could hash the project ID to get a consistent color
  ...
  useEffect(() => {
    ...
    // For now, refresh on focus or interval could be enough
  }, [fetchData]);
  ...
  const startTask = async (id: string) => {
    ...
    if (!window.confirm(`Move "${task.title || "task"}" to In Progress?`)) return;

    // Optimistic update
    setTasks((prev) => prev.map((t) => (t.id === id ? { ...t, status: "in_progress" as const } : t)));

    try {
      const result = await updateTaskStatusAction(task.projectId, id, "in_progress");
      if (!result.success) throw result.error;

      showToast(`Started "${task.title}".`, "info", 7000, {
        label: "Undo",
        onClick: async () => {
          setTasks((prev) => prev.map((t) => (t.id === id ? { ...t, status: "todo" as const } : t)));
          await updateTaskStatusAction(task.projectId, id, "todo");
          showToast(`Reverted "${task.title}".`, "success", 2500);
        },
      });
    } catch (error) {
      // Revert on error
  ...
  const completeTask = async (id: string) => {
    const task = tasks.find((t) => t.id === id);
    if (!task) return;
    if (!window.confirm(`Move "${task.title || "task"}" to Done?`)) return;

    const prevStatus = task.status;

    // Optimistic remove (list hides done tasks)
    setTasks((prev) => prev.filter((t) => t.id !== id));

    try {
      const result = await updateTaskStatusAction(task.projectId, id, "done");
      if (!result.success) throw result.error;

      showToast(`Marked "${task.title}" done.`, "success", 7000, {
        label: "Undo",
        onClick: async () => {
          // Restore locally
          setTasks((prev) => [task, ...prev]);
          await updateTaskStatusAction(task.projectId, id, prevStatus);
          showToast(`Reverted "${task.title}".`, "success", 2500);
        },
      });
    } catch (error) {
