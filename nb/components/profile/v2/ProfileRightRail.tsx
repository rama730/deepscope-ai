...
function normalizeSocialLinks(profile: any, list: any[]): Array<{ label: string; url: string }> {
  ...
  // New schema: profiles.social_links JSONB
  const json = profile?.social_links;
  ...
  // Legacy table: social_links rows
  for (const row of list || []) {
    const url = String(row?.url || "").trim();
    const label = String(row?.platform || row?.label || "Link").trim();
    if (!url) continue;
    out.push({ label, url });
  }
