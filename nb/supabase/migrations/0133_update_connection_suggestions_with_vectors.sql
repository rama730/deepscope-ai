-- ==============================================================================
-- MIGRATION 0133: UPDATE CONNECTION SUGGESTIONS WITH VECTOR SEARCH
-- Updates get_connection_suggestions to include semantic similarity score
-- ==============================================================================
...
BEGIN
  ...
  -- Get user's normalized location and embedding
  SELECT p.location_city, p.location_region, p.location_country, p.embedding
  INTO user_location_city, user_location_region, user_location_country, user_embedding
  FROM profiles p
  ...
  WITH candidate_profiles AS (
    SELECT DISTINCT p.id, p.username, p.full_name, p.avatar_url, p.bio, p.location, p.headline,
      p.location_city, p.location_region, p.location_country, p.embedding
    FROM profiles p
    WHERE p.id != user_uuid
  ...
  scored_profiles AS (
    SELECT
      cp.*,
      (
        -- Mutual connections score (highest weight: 15 per connection)
        COALESCE((
          SELECT COUNT(*)
        ...
        +
        -- Shared skills score (10 per skill)
        COALESCE((
          SELECT COUNT(*)
          FROM skills s1
        ...
        +
        -- Shared techniques score (8 per technique)
        COALESCE((
          SELECT COUNT(*)
        ...
        +
        -- Vector Similarity Score (AI Matching)
        -- Cosine distance (1 - distance) gives similarity 0-1.
        -- We multiply by 50 to give it significant weight (up to 50 points)
        -- Only applies if both users have embeddings
        CASE 
          WHEN user_embedding IS NOT NULL AND cp.embedding IS NOT NULL 
          THEN (1 - (user_embedding <=> cp.embedding)) * 50
          ELSE 0
        END
        +
  ...
  computed_reasons AS (
    SELECT
      ...
      -- Use vector similarity as a reason if it's high
      CASE 
        WHEN user_embedding IS NOT NULL AND sp.embedding IS NOT NULL AND (1 - (user_embedding <=> sp.embedding)) > 0.85 
        THEN true 
        ELSE false 
  ...
  SELECT
    ...
    jsonb_build_object(
      ...
      'semantic_match', cr.semantic_match, -- New reason
