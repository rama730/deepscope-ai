...
begin
  -- 1. Get Core Profile Data (Explicitly select safe columns)
  ...
  -- 3. Get Skills (Sorted by display_order)
  select jsonb_agg(s order by s.display_order asc, s.is_featured desc, s.endorsement_count desc)
  into v_skills
  from skills s
  where s.user_id = p_user_id;

  -- 4. Get Experience (Sorted by display_order then date)
  select jsonb_agg(e order by e.display_order asc, e.start_date desc)
  into v_experiences
  from experiences e
...
-- Grant access to anon for public profiles
grant execute on function get_profile_details(uuid) to anon;
