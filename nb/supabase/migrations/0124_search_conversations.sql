-- Create function to search user conversations
...
BEGIN
  ...
  WHERE (
      ...
      cd.group_name ILIKE '%' || p_query || '%' OR
      pi.title ILIKE '%' || p_query || '%' OR
      op.full_name ILIKE '%' || p_query || '%' OR
      op.username ILIKE '%' || p_query || '%'
  )
